...
<style>
  ...
  .theme-summary .content-container { max-width: 1120px; }
  ...
  .section-heading::after {
      ...
      width: 40px; height: 1px; background: #c5a059;
  }
  .section-lead { text-align: center; font-weight: 500; margin-bottom: 60px; color: #666; }
  ...
  .offer-badge {
      ...
      padding: 8px 30px; font-size: 13px; letter-spacing: 2px;
      box-shadow: 0 5px 15px rgba(197, 160, 89, 0.2);
      white-space: nowrap;
  }
  .offer-title {
      font-family: 'Noto Serif JP', serif; font-size: 24px; color: #8e6e2e; margin-bottom: 20px; line-height: 1.6;
  }
  .offer-desc { font-size: 14px; line-height: 2; color: #555; margin-bottom: 40px; }
  .marker { background: linear-gradient(transparent 70%, #ffeb3b 70%); font-weight: 700; color: #333; }

  .price-comparison {
      display: flex; justify-content: center; align-items: center; gap: 40px; margin-bottom: 30px; flex-wrap: wrap;
  }
  .comp-col .label { display: block; font-size: 11px; color: #888; margin-bottom: 5px; letter-spacing: 1px; }
  .comp-col.regular .value { font-family: 'Playfair Display', serif; font-size: 24px; color: #aaa; text-decoration: line-through; }
  .comp-col.special .value { font-family: 'Playfair Display', serif; font-size: 42px; color: #c0392b; line-height: 1; font-weight: 500; }
  .comp-col .tax { font-size: 12px; color: #666; font-weight: normal; margin-left: 5px; font-family: 'Noto Serif JP', serif; }
  .comp-arrow { font-size: 24px; color: #c5a059; font-weight: 300; }
  .limit-text { font-size: 12px; color: #c0392b; font-weight: 700; letter-spacing: 0.05em; }

  .price-grid {
      display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px; align-items: flex-end;
  }
  .price-card {
      background: #fff; border: 1px solid #eee; padding: 40px 30px; text-align: center;
      transition: all 0.4s ease; position: relative; display: flex; flex-direction: column;
  }
  .price-card:hover {
      border-color: #c5a059; box-shadow: 0 15px 40px rgba(0,0,0,0.05); transform: translateY(-5px);
  }
  .price-card.recommended {
      border: 1px solid #c5a059; padding-top: 60px; transform: scale(1.05); z-index: 2;
      box-shadow: 0 20px 50px rgba(197, 160, 89, 0.1);
  }
  .price-card.recommended:hover { transform: scale(1.05) translateY(-5px); }
  .price-card--gold { border: 1px solid #c5a059; background: #fffaf0; }
  .price-card--gold:hover { border-color: #c5a059; }

  .rec-label {
      position: absolute; top: 0; left: 0; width: 100%;
      background: #c5a059; color: #fff; font-size: 10px; letter-spacing: 2px;
      padding: 6px 0; font-weight: 600; font-family: 'Lato', sans-serif;
  }
  .card-header { margin-bottom: 25px; }
  .plan-name { display: block; font-family: 'Playfair Display', serif; font-size: 20px; color: #333; margin-bottom: 5px; font-weight: 600; }
  .plan-sub { display: block; font-size: 11px; color: #888; }
  .card-price { font-family: 'Playfair Display', serif; font-size: 28px; color: #333; margin-bottom: 30px; letter-spacing: 0.05em; }
  .card-price.highlight { color: #c0392b; font-size: 32px; font-weight: 500; }
  .card-price .tax { font-size: 11px; color: #999; font-family: 'Noto Serif JP', serif; font-weight: normal; }

  .plan-features { text-align: left; margin-bottom: 30px; border-top: 1px solid #f9f9f9; padding-top: 25px; list-style: none; }
  .plan-features li { font-size: 13px; color: #555; margin-bottom: 12px; padding-left: 20px; position: relative; line-height: 1.6; }
  .plan-features li::before { content: '✔'; position: absolute; left: 0; color: #c5a059; font-size: 10px; top: 2px; }
  .plan-features li.bonus { font-weight: 700; color: #c0392b; }
  .plan-note { font-size: 12px; color: #c0392b; font-weight: 600; margin: 0 0 20px; text-align: left; }
  .plan-features li.disabled { color: #ccc; text-decoration: line-through; }
  .plan-features li.disabled::before { content: '-'; color: #ccc; }

  .btn-plan {
      display: block; width: 100%; padding: 16px 0; background: #333; color: #fff;
      font-size: 13px; letter-spacing: 1px; border: 1px solid #333; margin-top: auto; text-decoration: none; transition: all 0.3s;
  }
  .btn-plan:hover { background: #fff; color: #333; }
  .btn-rec { background: #c5a059; border-color: #c5a059; font-weight: 600; }
  .btn-rec:hover { color: #c5a059; background: #fff; }

  .price-card.retake {
      grid-column: 1 / -1; flex-direction: row; justify-content: space-between; align-items: center;
      background: #fafafa; border: 1px dashed #ccc; margin-top: 40px; padding: 30px 50px;
  }
  .retake-info { text-align: left; }
  .retake-info .plan-name { font-size: 16px; margin-bottom: 5px; }
  .retake-info p { font-size: 12px; color: #666; }
  .retake-right { display: flex; align-items: center; gap: 40px; flex-wrap: wrap; }
  .retake-price { font-family: 'Playfair Display', serif; font-size: 14px; display: flex; flex-direction: column; gap: 12px; line-height: 1.5; }
  .retake-price-line { display: block; white-space: nowrap; font-size: 14px; color: #333; }
  .retake-price-line .tax { font-size: 12px; color: #666; }
  .retake-buttons { display: flex; flex-direction: column; gap: 10px; min-width: 200px; }
  .price-card.retake .btn-simple { font-size: 12px; padding: 12px 24px; white-space: normal; line-height: 1.4; }
  .btn-simple { text-decoration: none; font-size: 13px; border: 1px solid #333; color: #333; padding: 10px 30px; transition: all 0.3s; text-align: center; white-space: nowrap; }
  .btn-simple:hover { background: #333; color: #fff; }
  .condition-note { font-size: 10px; color: #c0392b; margin-bottom: 15px; }

  .retake-notes {
      grid-column: 1 / -1; margin-top: 20px; padding: 25px 30px;
      background: #fafafa; border: 1px solid #eee; font-size: 13px; color: #555; line-height: 1.9;
  }
  .retake-notes p { margin: 0 0 12px; }
  .retake-notes p:last-child { margin-bottom: 0; }

  .schedule-filters { display: flex; justify-content: center; gap: 10px; margin-bottom: 50px; flex-wrap: wrap; }
  .filter-btn {
      background: transparent; border: 1px solid #eee; padding: 8px 24px; border-radius: 30px;
      font-family: 'Playfair Display', serif; font-size: 13px; cursor: pointer; transition: all 0.3s ease; color: #888;
  }
  .filter-btn.active { background: #333; border-color: #333; color: #fff; }

  .month-block { display: flex; align-items: flex-start; margin-bottom: 50px; border-top: 1px solid #eee; padding-top: 40px; }
  .month-block.hidden { display: none; }
  .month-label {
      width: 150px; flex-shrink: 0; font-family: 'Playfair Display', serif; font-size: 32px;
      color: #c5a059; text-align: right; padding-right: 40px; border-right: 1px solid #eee; line-height: 1; margin-top: 5px;
  }
  .schedule-list { flex-grow: 1; padding-left: 40px; list-style: none; }
  .schedule-list li {
      display: flex; align-items: center; margin-bottom: 15px; font-size: 14px;
      border-bottom: 1px solid #fafafa; padding-bottom: 15px;
      transition: all 0.4s ease;
  }
  .schedule-list li.hide { display: none; }
  .schedule-list .date {
      font-family: 'Playfair Display', serif; font-weight: 500; width: 120px; font-size: 16px; color: #333; flex-shrink: 0;
  }
  .schedule-list .wday { font-size: 12px; font-family: 'Noto Serif JP', serif; color: #888; margin-left: 3px; }

  .item-content { white-space: nowrap; }
  .tag {
      display: inline-block; font-size: 10px; padding: 3px 10px; margin-right: 15px; min-width: 70px;
      text-align: center; letter-spacing: 1px; font-family: 'Lato', sans-serif; font-weight: 700; border-radius: 2px;
  }
  .tag.basic { background: #eef9f6; color: #6c9a93; border: 1px solid #6c9a93; }
  .tag.adv { background: #fff5eb; color: #c5a059; border: 1px solid #c5a059; }
  .tag.ws { background: #f0f7ff; color: #e67e22; border: 1px solid #e67e22; }
  .req { font-size: 10px; color: #c0392b; font-weight: 700; margin-left: 10px; }
  .hybrid-label { color: #c0392b; font-size: 0.75em; margin-left: 2px; }
  .schedule-note { font-size: 12px; color: #888; margin-top: 16px; margin-left: 120px; padding-top: 12px; border-top: 1px dashed #eee; }
  .schedule-note.accent { color: #c0392b; font-weight: 600; }

  .terms-box { background: #fafafa; padding: 50px; border: 1px solid #eee; }
  .term-title {
      font-size: 16px; margin-bottom: 15px; padding-left: 15px;
      border-left: 3px solid #c5a059; color: #2c3e50; letter-spacing: 0.05em; font-weight: 700;
  }
  .term-list { padding-left: 15px; margin-bottom: 40px; }
  .term-list li { list-style-type: disc; margin-left: 20px; margin-bottom: 8px; font-size: 14px; color: #666; line-height: 1.8; }

  .cta-area { text-align: center; border-top: 1px solid #eee; padding-top: 80px; margin-top: 100px; padding-bottom: 100px; }
  .cta-heading { font-family: 'Playfair Display', serif; font-size: 36px; margin-bottom: 10px; color: #333; }
  .payment-buttons { max-width: 600px; margin: 30px auto; display: flex; flex-direction: column; gap: 20px; }
  .pay-btn {
      display: flex; justify-content: space-between; align-items: center; padding: 25px 40px;
      border: 1px solid #ccc; background: #fff; transition: all 0.3s; text-decoration: none;
  }
  .pay-btn:hover { border-color: #333; background: #fafafa; transform: translateX(5px); }
  .pay-btn.gold { background: #fffaf0; border-color: #c5a059; }
  .pay-btn.gold .btn-main { color: #8e6e2e; font-weight: 700; }
  .btn-sub { display: block; font-size: 11px; color: #888; margin-bottom: 5px; text-align: left; }
  .btn-main { display: block; font-size: 16px; font-weight: 500; color: #333; text-align: left; }
  .pay-btn-price { font-family: 'Playfair Display', serif; font-size: 20px; color: #333; }
  .pay-btn-note { display: block; font-size: 12px; color: #c0392b; font-weight: 600; margin-top: 6px; text-align: left; }
  ...
  @media (max-width: 768px) {
      ...
      .special-offer-box { padding: 40px 20px; }
      .offer-title { font-size: 20px; }
      .price-comparison { gap: 15px; }
      .comp-col.regular .value { font-size: 18px; }
      .comp-col.special .value { font-size: 32px; }
      .comp-arrow { transform: rotate(90deg); margin: 5px 0; }
      .price-grid { grid-template-columns: 1fr; gap: 40px; }
      .price-card.recommended { order: -1; transform: none; padding-top: 50px; }
      .price-card.retake { flex-direction: column; padding: 30px 20px; gap: 20px; align-items: stretch; }
      .retake-right { flex-direction: column; gap: 20px; align-items: stretch; }
      .retake-price { font-size: 13px; text-align: left; }
      .price-card.retake .btn-simple { white-space: normal; text-align: center; }
      .schedule-filters { gap: 8px; }
      .filter-btn { padding: 6px 16px; font-size: 12px; }
      .month-block { flex-direction: column; border-top: none; padding-top: 0; }
      .month-label {
          width: 100%; text-align: left; border-right: none; border-bottom: 1px solid #eee;
          padding: 0 0 10px; margin-bottom: 20px; font-size: 24px;
      }
      .schedule-list { padding-left: 0; }
      .schedule-list li { flex-direction: column; align-items: flex-start; gap: 8px; }
      .schedule-list .date { width: 100%; margin-bottom: 0; font-size: 18px; }
      .item-content { white-space: normal; }
      .schedule-note { margin-left: 0; }
      .terms-box { padding: 30px 20px; }
      .pay-btn { padding: 20px; flex-direction: column; text-align: center; gap: 15px; }
      .pay-btn:hover { transform: none; }
      .btn-sub, .btn-main { text-align: center; }
      .pay-btn-price { width: 100%; justify-content: center; border-top: 1px solid #eee; padding-top: 10px; }
  }
</style>
    ...
    <div class="content-container">
      ...
      <section class="detail-section js-anim-section">
        ...
        <div class="special-offer-box js-anim-item">
          ...
          <p class="limit-text">【早割セット価格】※5/7(木)申込分まで 対面1Dayワークショップ2回分(¥66,000相当)無料ご招待の対象<br>
          <br>
          ▼対象ワークショップ<br>
          ・9/22(火) 11:00〜18:00　テーマ：『エネルギー・アタッチメント ― 他者のエネルギーを手放す―』<br>
          ・9/23(水) 11:00〜18:00　テーマ：『本当の私を生きる ― 真の自分を表現する勇気』</p>
        ...
        <div class="price-grid js-anim-item">
          ...
          <div class="price-card price-card--gold">
            ...
            <p class="plan-note">※早割の適用は5/7（木）23:59まで</p>
        ...
        <div class="payment-buttons js-anim-item">
          ...
          <a href="https://www.ohnoyuriko.com/event-details/tajigentogoryoho-beshikkukosutasuadobansukosu-zen10nichikan-1" target="_blank" rel="noopener" class="pay-btn gold">
            <div class="pay-btn-info">
              ...
              <span class="pay-btn-note">※早割の適用は5/7（木）23:59まで</span>
            </div>
            <div class="pay-btn-price">
